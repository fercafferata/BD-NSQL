...
const initialHeroes = [
  ...
  },
  {
    ...
    images: "/images/doctor-midnite.jpg"
  },
  {
...
export const initializeData = async () => {
  try {
    ...
    if (heroCount === 0) {
      ...
      const insertedHeroes = await Hero.insertMany(initialHeroes)

      console.log(`✅ Se insertaron ${insertedHeroes.length} superhéroes iniciales:`)
      insertedHeroes.forEach((hero) => {
        console.log(`   - ${hero.name} (${hero.publisher.toUpperCase()})`)
      })
    } else {
      console.log(`📊 Base de datos ya contiene ${heroCount} superhéroes`)
    }
  } catch (error) {
